...
def validate_full(args):
    ...
    # Coerce boolean-like flags to real bools (they can arrive as strings via config passthrough).
    # None = flag omitted -> default True (matches the previous fallback behavior).
    args.epi_mode = True if args.epi_mode is None else as_bool(args.epi_mode)
    args.use_cladebreaker = True if args.use_cladebreaker is None else as_bool(args.use_cladebreaker)
    ...
    if not args.epi_mode:
        print("Epi mode disabled. Clusters will not be determined.")
    else:
        ...
        # If any patient_id is missing, disable epi mode and print a warning message
        if patient_missing:
            print("Patient ID is not complete. Disabling epi mode and no clusters will be determined.")
            args.epi_mode = False
        else:
            unique_patient_count = input_df['patient_id'].nunique(dropna=True)
            print(f"Number of unique patient IDs: {unique_patient_count}")
        
        # Check collection_date column, if any collection_date is missing, disable epi mode
        if date_missing:
            print("Collection date column contains missing values. Disabling epi mode and no clusters will be determined.")
            args.epi_mode = False
        else:
    ...
    if args.endpoint not in {"plateau", "peak", "discrepancy", "public"}:
    ...
    if not args.threads:
        ...
        args.threads = 1
# Validate arguments for redo-endpoint mode
def validate_redo_endpoint(args):
    """Validate the arguments used in redo-endpoint function"""
    ...
    if not args.thresher_output or not os.path.exists(args.thresher_output):
        ...
        # Look for peak_strains.RDS, plateau_strains.RDS, public_strains.RDS, and discrepancy_strains.RDS within the existing THRESHER directory
        thresher_output = {
            ...
            "public": os.path.join(args.thresher_output, "thresher", "output", "public_strains.RDS"),
    ...
    if not os.path.exists(args.output):
        ...
        os.makedirs(args.output)
        args.output = os.path.abspath(args.output)
# Validate arguments for new-snps mode
def validate_new_snps(args):
    """Validate the arguments used in new-snps function"""
    ...
    if not args.thresher_output or not os.path.exists(args.thresher_output):
        ...
        thresher_mandatory_output = {
            ...
            "public_strains": os.path.join(args.thresher_output, "thresher", "output", "public_strains.RDS"),
    ...
    if not args.output:
        ...
        args.output = os.path.abspath(os.path.join(os.getcwd(), f"thresher_new_snps_{args.prefix}"))
    if not os.path.exists(args.output):
        print(f"Output directory {args.output} does not exist, creating it.")
        os.makedirs(args.output)
        args.output = os.path.abspath(args.output)
...
def validate_new_full(args):
    ...
    if not args.thresher_output or not os.path.exists(args.thresher_output):
        ...
        thresher_mandatory_output = {
            ...
            # Prior SNP matrix from the study genomes
            "study_snp_matrix": os.path.join(args.thresher_output, "mummer4_study", "study_snp_matrix.RDS"),
            # Prior SNP matrix from the public genomes
            "public_snp_matrix": os.path.join(args.thresher_output, "mummer4_public", "public_snp_matrix.RDS"),
    ...
    if not args.output:
        ...
        args.output = os.path.abspath(os.path.join(os.getcwd(), f"thresher_new_full_{args.prefix}"))
    ...
    if args.endpoint == "plateau" and not args.plateau_length:
        print("Plateau length not provided, using default length of 15")
        args.plateau_length = 15
# Validate arguments for cladebreaker_off mode
def validate_cladebreaker_off(args):
    """Validate the arguments used in cladebreaker_off function"""
    # Check prefix
    if not args.prefix:
        print("Prefix not provided, using default prefix: current timestamp in the format of YYYY_MM_DD_HHMMSS")
        args.prefix = time.strftime("%Y_%m_%d_%H%M%S")
    ...
    if not args.output:
        print(f"Output directory not provided, creating a directory named 'thresher_cladebreaker_off_{args.prefix} under current working directory({os.getcwd()}) as output directory")
        args.output = os.path.abspath(os.path.join(os.getcwd(), f"thresher_cladebreaker_off_{args.prefix}"))
    if not os.path.exists(args.output):
        print(f"Output directory {args.output} does not exist, creating it.")
        os.makedirs(args.output)
        args.output = os.path.abspath(args.output)
