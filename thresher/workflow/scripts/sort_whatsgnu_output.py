...
public_genome_dir = snakemake.params.public_genome_dir
...
for file in whatsgnu_output_file:
    ...
    try:
        public_genomes_df = pd.read_csv(file, sep='\t', header=None, skiprows=1)
        if public_genomes_df.empty:
            print(f"No top genomes found for {study_genome}, skipping.")
            continue
        public_genomes_df = public_genomes_df.iloc[:, 0]
        public_genomes = [genome for genome in public_genomes_df if 'GCA_' in genome]
    ...
    extracted = set()
    for genome_entry in public_genomes:
    ...
    public_genomes = list(extracted)
    
    # Only keep the public genomes that are not the same as the study genome
    # Also ensure they are in the actual downloaded top genomes
    public_genomes = [genome for genome in public_genomes if genome in actual_download_topgenomes]
    public_genomes = [genome for genome in public_genomes if genome not in study_accession]
    
    with open(os.path.join(script_path, f'dnadiff_{study_genome}.sh'), 'w') as f:
        if len(public_genomes) > 0:
            f.write(f"mkdir -p {output_path}/{study_genome}\n")
            f.write(f"cd {output_path}/{study_genome}\n")
            for public_genome_entry in public_genomes:
                f.write(f"dnadiff {study_genome_dict[study_genome]} {public_genome_dir}/{public_genome_entry}.fna -p s_{study_genome}_q_{public_genome_entry} \n")
                f.write(f"sed -n '1p; /AlignedBases/p; /TotalSNPs/p; /TotalGSNPs/p' s_{study_genome}_q_{public_genome_entry}.report > sorted_s_{study_genome}_q_{public_genome_entry}.report\n")
                f.write(f"dnadiff {public_genome_dir}/{public_genome_entry}.fna {study_genome_dict[study_genome]} -p s_{public_genome_entry}_q_{study_genome} \n")
                f.write(f"sed -n '1p; /AlignedBases/p; /TotalSNPs/p; /TotalGSNPs/p' s_{public_genome_entry}_q_{study_genome}.report > sorted_s_{public_genome_entry}_q_{study_genome}.report\n")
            ...
            f.write(f"echo 'No public genomes found for {study_genome} other than itself.'\n")
