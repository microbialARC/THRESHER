...
new_clusters_summary <- lapply(unique(strains_rds[["strains"]]$strain_id), function(strain) {
...
genomes_summary_csv <- do.call(rbind,
                               lapply(strains_rds[["strains"]]$genome,
                                      function(genome_entry){
                                        # Is this genome a new genome
                                        is_new_genome <- genome_entry %in% new_metadata$V1
                                        
                                        # Strain ID of this genome
                                        genome_strain_id <- strains_rds[["strains"]]$strain_id[strains_rds[["strains"]]$genome == genome_entry]
                                        
                                        # Cluster ID of this genome
                                        genome_cluster_id <- unlist(sapply(new_clusters_summary,
                                        ...
                                        if(length(genome_cluster_id) == 0){
                                          genome_cluster_id <- "Non-Cluster"
                                        }
                                        
                                        return(
                                          data.frame(
                                            genome = genome_entry,
                                            category = if(is_new_genome) "new" else "original",
                                            strain = genome_strain_id,
                                            cluster = genome_cluster_id
                                          )
                                        )
                                      }))
