...
thresher_MSTP <- function(output_path,
  ...
  for(i in seq_along(thresher_input)){
    # If length(thresher_input) == 1, this is singleton group 
    # No MSTP will be generated
    if(length(thresher_input[[i]]) > 1){
      group_id <- thresher_input[[i]][[1]]$HC_group
      group_input <- thresher_input[[i]]
      ...
      combine_MSTPs[[paste0("Group", group_id)]] <- combined_plot
