...
thresher_qc <- function(method_strain_path,
  ...
  # Read everything tree and generate the distance matrix
  everything_tree <- read.tree(everything_tree_path)
  everything_tree <- phytools::midpoint_root(everything_tree)
  everything_tree <- Preorder(everything_tree)
  tree_distance_matrix <-  cophenetic.phylo(everything_tree)
...
everything_tree_path <- snakemake@input[["everything_tree"]]
