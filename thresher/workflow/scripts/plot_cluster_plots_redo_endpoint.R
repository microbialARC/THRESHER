...
mrsa_result_path <- file.path(thresher_output,"blastx","mrsa","output","summary","blastx_MRSA_strains.csv")
if(file.exists(mrsa_result_path)){
  mrsa_results <- read.csv(mrsa_result_path)
}
...
clusters_summary <- lapply(unique(strains_rds[["strains"]]$strain_id), function(strain) {
  ...
  if(length(strain_patients) > 1) {
    return(list(
      ...
      AMR = if(file.exists(mrsa_result_path)) mrsa_results$MRSA[mrsa_results$strain == strain] else "N/A",
...
dirs <- file.path(output_dir, c("plots", "files"))
