rule mummer4_public_new_full_cmd:
    input:
    ...
    output:
        [os.path.join(config["output"],"mummer4_public","scripts",f"dnadiff_{genome}.sh") for genome in list(new_genome_path_dict.keys())]
    params:
...
rule mummer4_public_new_full_single:
    conda:
...
rule mummer4_public_new_full_all:
    input:
        reports = expand(os.path.join(config["output"], "mummer4_public","{genome_name}_concatenated.report"), genome_name=new_genome_path_dict.keys())
    output:
