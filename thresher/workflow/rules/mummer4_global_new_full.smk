...
rule mummer4_global_new_full:
    conda:
        os.path.join(BASE_PATH,"envs/mummer4.yaml")
    input:
        script = [os.path.join(config["output"],"mummer4_global","scripts",f"dnadiff_{genome}.sh") for genome in list(new_genome_path_dict.keys())]
    output:
