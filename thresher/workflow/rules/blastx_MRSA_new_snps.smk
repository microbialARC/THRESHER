rule blastx_mrsa_new_snps_cmd:
    ...
    output:
        blastx_mrsa_new_snps_cmd = expand(os.path.join(config["output"],"blastx","mrsa","scripts","{genome_name}_blastx_mrsa.sh"), genome_name=new_genome_path_dict.keys())
    shell:
        """
...
rule blastx_mrsa_new_snps_single:
    conda:
        os.path.join(BASE_PATH,"envs/blast.yaml")
    input:
        blastx_mrsa_new_snps_single_cmd = os.path.join(config["output"],"blastx","mrsa","scripts","{genome_name}_blastx_mrsa.sh")
    output:
        blastx_mrsa_new_snps_single_result = os.path.join(config["output"],"blastx","mrsa","output","raw","{genome_name}_blastx_mrsa.tsv")
    shell:
        """
        bash {input.blastx_mrsa_new_snps_single_cmd}
        """

rule blastx_mrsa_new_snps_results:
    conda:
